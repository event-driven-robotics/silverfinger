...
startElevation = 5 # up from horizontal, out from centre
sensorPitch = 1

# modelling directives
spiralEndIds = np.array([80, 90, 100, 110, 250, 260, 290, 300])
spiralEndIdsActual = np.ones((numSpirals), dtype=np.int) * 10000
direction = np.array([1.2, 3.2, 0.2, 2.2, 0.1, 2.6, -0.2, 1.8])[np.newaxis, :] * 90
xOffset = np.array([-20, 20, 20, -20, -15, 15, 20, -20])
yOffset = np.array([20, -20, 15, -15, 20, -20, 10, -10])
...
relaxationOuter = 1 # factor by which outer spiral should twist outwards to restore the ideal space filling
relaxationInner = 0.2 # factor by which outer spiral should twist outwards to restore the ideal space filling
...
while np.any(running[-1, :]):
        ...
        for spiralIdx in range(numSpirals):
            if running[-1, spiralIdx]:
                newElevationFromCentre = max(
                    ...
                    relaxationOuter * outerRadius[-1, spiralIdx] 
                ...
                newInnerRadius = min(
                    ...
                    relaxationInner * minPitchBetweenTouchingLayers * 
    ...
    cumDistOuter = cumDistOuter + distanceOuter
